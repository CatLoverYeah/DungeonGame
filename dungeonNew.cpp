...
int GetInt(){int v; if(!(cin>>v)){cin.clear();cin.ignore(10000,'\n');return -1;} return v;}
string GetString(){string s; if(!(cin>>s)){cin.clear();cin.ignore(10000,'\n');return "";} return s;}
...
bool hasSuperDuperFancyCoolMegaSword = false;
bool hasSeenRules = false;
int woodenSwordStamina = 18;
int skeletonHP = 100;
int punchStamina = 8;
int bowStamina = 13;
int electricSpellStamina = 30;
int superDuperFancyCoolMegaSwordStamina = 24;
int playerHP = 150;
int stamina = 200;
int bowDamage = 25;
...
void Room13Loop();
void GameRulesLoop();
void SkeletonFightLoop();
void GameRules1Loop();
void TreasureRoom2Loop();

int main() {
  srand(time(NULL));
  if(hasSeenRules == false) {
...
void Rooms() {
    ...
    if (!(cin >> ans1)) {
        ...
        cout << "Sorry, that is not an option, try again\n\n";
        Sleep(1400);
        Rooms();
        return;
    }
...
void GameRules() {
  string jk = GetString();
  if(jk == "yes" || jk == "Yes") {
    cout << "The purpose of the game is to collect items to beat the final monster at the end of the game\n";
    Sleep(3600);
    cout << "You have a HP bar and stamina that will display every fight you play\n";
    Sleep(2600);
    cout << "Your HP doesn't refresh after a fight, so find food to replenish it\n";
    Sleep(3500);
    cout << "Your stamina also doesn't replenish after a fight, so also find collectables for it\n";
    Sleep(3000);
    cout << "BUT\n";
    Sleep(800);
    cout << "Stamina does go up 5 every time it's your turn in a fight\n\n";
    Sleep(2800);
...
void GameRules1Loop() {
  cout << "Right, so money is also very important int the game\n";
}
...
void GameRulesLoop() {
  string gj = GetString();
    if(gj == "yes" || gj == "Yes") {
      cout << "Great! so...\n";
      Sleep(1000);
      cout << "The whole point of the game is to beat all the monsters and minibosses\n";
      Sleep(3500);
...
int hk = GetInt();
  if(hk == 1 || hk == 2 || hk == 3 || hk == 4 || hk == 5 || hk == 6 || hk == 7 || hk == 8 || hk == 9 || hk == 0) {
    ...
    int punchStamina = 10;
    int bowStamina = 8;
    int electricSpellStamina = 25;
    int superDuperFancyCoolMegaSwordStamina = 20;
    int playerHP = 150;
    int stamina = 200;
    int bowDamage = 25;
    int woodenSwordDamage = 35;
